...
class Hyperparameters:
    ...
    learning_rate : float = 0.004 # Conservative starting point for single AdamW
...
# Single optimizer for all parameters (no Muon)
...
optimizers = [optimizer]  # keep as list for compatibility
...
def get_lr(it):
    ...
    # 3) constant lr (or optional boost during early phase)
    else:
        # Optional: boost LR during early orthogonal phase
        # if it < args.num_iterations // 16:
        #     return 1.4  # 40% higher during orthogonal re-projection phase
        # else:
...
# Optional: Orthogonal re-projection schedule
# Uncomment to enable periodic re-projection during early training
ORTHO_REPROJECT_EVERY = 100  # Less frequent than with Muon
ORTHO_REPROJECT_UNTIL = 500  # Only during first 500 steps
...
for step in range(args.num_iterations + 1):
    ...
    # Optional: Periodically re-project weights to orthogonal manifold
    # Uncomment the following block to enable light re-projection during early training
    # if step > 0 and step % ORTHO_REPROJECT_EVERY == 0 and step < ORTHO_REPROJECT_UNTIL:
